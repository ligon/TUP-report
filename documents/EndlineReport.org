...
make comparisons of total consumption misleading.  However,
...
*** Recreating consumption tables                                    :ignore:
CTL mean lists the mean and standard deviation of the control group 
in 2015.  NaN values were considered as having a value of 0 for that good (this occurs
automatically when using .sum(axis=1) to sum values in different
columns for a given row). None of the monthly or yearly variables were
divided by 30 or 365 respectively, and similarly none of the food
variables were transformed. "Monthly" is the sum of all the
goods which were asked about with a 1-month recall window, and
"Yearly" is the sum of all the goods which were asked about with a
1-year long recall window. "FoodShr" is the amount of "Food" divided
by "Total", which could have strange interpretations due to all the
different length recall windows and lack of transforming the variables
to all be "per day consumption" for example

Differences with previous code: 
-This code does not do any censoring, truncating, or converting of
values to US dollars
-This code added s20_6m_b and s20_7m_b in monthly for baseline as
c_othermonth_b (missing in previous code)
-The previous code was missing durables from monthly for midline. This
code adds it in
-The previous code constructed "total" by multiplying food consumption
by 3 due to a mistake with filtering. This was changed in this new code
-This code does not replace 0s with np.nan as was done in previous
code - for total, there seem to be 4 observations that have 0
-The new tables include "Monthly" for all the goods that were asked
about with a 1-month recall window and "Yearly" for all the goods that
were asked about with a 1-year recall window (the values were not
transformed by dividing by 30 or 365 or anything)
...
-Consider that it doesn't make sense to ask for clothing and footwear
in total and then women's clothes, children's clothes, school
uniforms, and shoes separately in midline and endline. Both the total
and the individual components are used in the sums here - change this?
Also the components do not always add up to the reported sum
-Consider what it means for the interpretation that different items
were asked about in nbaseline, midline, and endline. For example,
multiple items were asked about in midline and endline that were not
asked about in baseline
-Consider whether transformations for recall windows would help the interpretation


#+name: consumption_tables
#+begin_src python :noweb no-export :exports none  :results output table raw labels=[] :colnames no :tangle consumption_tables.py
data_assignment  = "../../TUP-report/documents/master_assignment.csv"
data_baseline = "../../TUP-data/data/Baseline/TUP_baseline.dta"
data_midline  = "../../TUP-data/Midline/TUP_midline.dta"
data_endline = "../../TUP-data/Endline/TUP_endline.dta"
...
for outcome in outcomes: 
    ...
    if baseline_na==True:
        ...
        temp_controls_FE = temp_controls_FE + ['Baseline missing']
        temp_controls_no_FE = temp_controls_no_FE + ['Baseline missing']
    
    temp_controls_FE = temp_controls_FE + ['Baseline Value']
    temp_controls_no_FE = temp_controls_no_FE + ['Baseline Value']
    ...
    b_FE[outcome] = models_ols_FE[outcome].params
    se_FE[outcome] = models_ols_FE[outcome].bse
    
    b_FE[outcome]['Control mean'] = controlgroup[controlgroup['2015'] == 1][outcome].mean()
    se_FE[outcome]['Control mean'] = controlgroup[controlgroup['2015'] == 1][outcome].std()      

    b_FE[outcome][r'$\beta_{TUP}^{2014}-\beta_{UCT}^{2015}$'] = models_ols_FE[outcome].t_test("TUP*2014 - UCT*2015 = 0").summary_frame()["coef"][0]
    se_FE[outcome][r'$\beta_{TUP}^{2014}-\beta_{UCT}^{2015}$'] = b_FE[outcome][r'$\beta_{TUP}^{2014}-\beta_{UCT}^{2015}$']/models_ols_FE[outcome].t_test("TUP*2014 - UCT*2015 = 0").summary_frame()["t"][0]

    b_FE[outcome][r'$\beta_{TUP}^{2015}-\beta_{UCT}^{2015}$'] = models_ols_FE[outcome].t_test("TUP*2015 - UCT*2015 = 0").summary_frame()["coef"][0]
    se_FE[outcome][r'$\beta_{TUP}^{2015}-\beta_{UCT}^{2015}$'] = b_FE[outcome][r'$\beta_{TUP}^{2015}-\beta_{UCT}^{2015}$']/models_ols_FE[outcome].t_test("TUP*2015 - UCT*2015 = 0").summary_frame()["t"][0]
    
b_FE = pd.DataFrame(b_FE)
se_FE = pd.DataFrame(se_FE)

print("#+name: tab:consumption_w_fe")
print("#+caption: Consumption Results with Treatment Group Fixed Effects")
print(df_to_orgtbl(b_FE, sedf = se_FE, float_fmt = '%.2f'))

print("""#+name: tab:consumption_wo_fe""")
print("""#+caption: Consumption Results without Treatment Group Fixed Effects""")
...
#+caption: Consumption Results with Treatment Group Fixed Effects
|                                         | Food         | Monthly      | Yearly        |
|-----------------------------------------+--------------+--------------+---------------|
| TUP*2014                                | 11.56^{**}   | 27.90        | 293.11^{**}   |
|                                         | (4.71)       | (22.68)      | (128.07)      |
| TUP*2015                                | -4.33        | 3.43         | -150.79       |
|                                         | (4.56)       | (21.98)      | (124.09)      |
| UCT*2014                                | -8.11        | -1.30        | -97.71        |
|                                         | (5.90)       | (28.41)      | (160.44)      |
| UCT*2015                                | 12.48^{**}   | 35.56        | 24.73         |
|                                         | (5.83)       | (28.08)      | (158.56)      |
| 2014                                    | 77.48^{***}  | 259.41^{***} | 808.30^{***}  |
|                                         | (5.33)       | (23.51)      | (125.90)      |
| 2015                                    | 106.76^{***} | 294.59^{***} | 1022.19^{***} |
|                                         | (5.00)       | (21.91)      | (115.48)      |
| TUP                                     | 7.23^{**}    | 31.33^{**}   | 142.32^{*}    |
|                                         | (3.04)       | (14.62)      | (82.54)       |
| UCT                                     | 4.37         | 34.26^{*}    | -72.98        |
|                                         | (3.80)       | (18.32)      | (103.42)      |
| Baseline missing                        | 18.85^{***}  | -13.31       | 17.33         |
|                                         | (7.12)       | (32.81)      | (180.06)      |
| Baseline Value                          | 0.07^{*}     | -0.05        | 0.04          |
|                                         | (0.04)       | (0.14)       | (0.03)        |
| Control mean                            | 115.55       | 288.60       | 1051.76       |
|                                         | (79.63)      | (311.21)     | (1939.71)     |
| $\beta_{TUP}^{2014}-\beta_{UCT}^{2015}$ | -0.92        | -7.65        | 268.38        |
|                                         | (8.41)       | (40.50)      | (228.68)      |
| $\beta_{TUP}^{2015}-\beta_{UCT}^{2015}$ | -16.81^{***} | -32.13       | -175.52       |
|                                         | (6.08)       | (29.28)      | (165.29)      |
