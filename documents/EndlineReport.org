...
df_l = pd.read_csv(data_locations)
df_a = pd.read_csv(data_asset_assign)
...
for i in range(len(df)): 
    # if something is not in asset_assign (group), should be 'control' in locations (Group) (1157)
    if pd.isnull(df.iloc[i, df.columns.get_loc('group')]):
        df.iloc[i, df.columns.get_loc('Group')]='Control'
    
    # if something is assigned in asset_assign (group), it should be the same in locations (2131)
    if pd.notna(df.iloc[i, df.columns.get_loc('group')]) & (df.iloc[i, df.columns.get_loc('group')] != df.iloc[i, df.columns.get_loc('Group')]):
...
data_assignment  = "../../TUP-data/csv/locations.csv"
import pandas as pd
import numpy as np
...
# merge baseline data with group column from locations.csv  based on respondent id/id number
df = pd.merge(df_b, df_a[ ['RespID', 'Group'] ], how='inner', left_on="idno", right_on="RespID")

df.iloc[92, df.columns.get_loc('Group')]='Control'
...
|                          |    Cash | Control |     TUP |
|--------------------------+---------+---------+---------|
| c_meat_b                 |  12.460 |  12.616 |  10.912 |
| c_fuel_b                 |  20.702 |  22.851 |  21.684 |
| c_clothesfootwear_b      | 252.097 | 240.157 | 230.840 |
| c_soap_b                 |  13.710 |  14.502 |  14.242 |
| c_fish_b                 |   7.048 |   7.516 |   7.053 |
| c_charities_b            |  10.790 |  10.829 |   8.721 |
| c_cereals_b              |  28.371 |  27.562 |  24.721 |
| c_transport_b            |   5.339 |   5.267 |   4.291 |
| c_cosmetics_b            |  16.702 |  20.459 |  21.270 |
| c_sugar_b                |   4.573 |   5.139 |   4.906 |
| c_egg_b                  |   3.403 |   3.288 |   3.016 |
| c_oil_b                  |   3.669 |   4.093 |   3.701 |
| c_ceremonies_b           |  57.726 |  48.060 |  50.504 |
| c_beans_b                |   2.766 |   2.089 |   2.783 |
| c_fruit_b                |   2.073 |   2.071 |   1.803 |
| c_textiles_b             |  76.315 |  56.217 |  54.561 |
| c_utensils_b             |  92.823 |  89.861 |  86.766 |
| c_dowry_b                | 468.589 | 458.548 | 444.057 |
| c_furniture_b            |  87.782 |  71.626 |  66.463 |
| c_salt_b                 |   1.363 |   1.342 |   1.262 |
| c_vegetables_b           |   4.089 |   4.630 |   4.135 |
| asset_val_smallanimals_b | 113.468 | 236.601 | 150.533 |
| asset_val_bicycle_b      |  97.661 | 109.075 |  96.520 |
| asset_val_radio_b        |  41.919 |  58.448 |  52.480 |
| asset_val_motorcycle_b   | 695.573 | 341.737 | 534.693 |
| asset_val_net_b          |  19.411 |  19.164 |  19.832 |
| asset_val_poultry_b      |  33.508 |  42.402 |  39.037 |
| asset_val_bed_b          | 274.032 | 241.270 | 249.262 |
| asset_val_chairtables_b  | 210.403 | 206.786 | 177.418 |
| asset_val_mobile_b       |  93.339 |  97.537 | 110.164 |
| asset_val_netITN_b       |   9.000 |   7.822 |   9.037 |
| asset_n_house_b          |   2.947 |   2.829 |   2.860 |
...
#+RESULTS: locations.csv_attrition
|         |    2013 |    2014 |    2015 |
|---------+---------+---------+---------|
| Cash    | 124.000 | 113.000 | 111.000 |
| Control | 280.000 | 266.000 | 347.000 |
| TUP     | 245.000 | 227.000 | 236.000 |
| All     | 649.000 | 606.000 | 694.000 |

|  |2013|   2014|   2015  |
|-
| Cash  | 124.000 | 112.000 | 110.000 |
| Control  | 280.000 | 219.000 | 261.000 |
| TUP  | 245.000 | 223.000 | 232.000 |
| All  | 649.000 | 554.000 | 603.000 |

# using checklist_20150602
#+name: checklist_20150602.csv_attrition
#+begin_src python :exports none  :results output table raw labels=[] :colnames no
data_baseline  = "../../TUP-data/Baseline/TUP_baseline.dta"
data_assignment  = "../../ElliottTUP/endline/checklists/checklist_20150602.csv"
data_midline = "../../TUP-data/Midline/TUP_midline.dta"
data_endline = "../../TUP-data/Endline/TUP_endline.dta"
import pandas as pd
import numpy as np
...
# baseline (2013)
# merge baseline data id number and group column from random assignment data based on respondent id/id number
df = pd.merge(df_b[ ['idno'] ], df_a[ ['RespID', 'Give'] ], how='inner', left_on="idno", right_on="RespID")

# change "Gift" to "Control"
df.replace(to_replace='Gift', value='Control', inplace=True)
df.replace(to_replace='First', value='Cash', inplace=True)
df.replace(to_replace='Second', value='Cash', inplace=True)

bygroupdf = df.groupby(['Give']).count()

d2013 = {'Cash': bygroupdf.at['Cash','idno'],'Control': bygroupdf.at['Control','idno'],'TUP': bygroupdf.at['TUP','idno'],'All': df.Give.notna().sum()}

df_b = df

# midline (2014)
# merge midline data id number and group column from random assignment data based on respondent id/id number
df = pd.merge(df_m[ ['idno'] ], df_a[ ['RespID', 'Give'] ], how='inner', left_on="idno", right_on="RespID")
...
d2014 = {'Cash': bygroupdf.at['Cash','idno'],'Control': bygroupdf.at['Control','idno'],'TUP': bygroupdf.at['TUP','idno'],'All': df.Give.notna().sum()}

df_m = df

# endline (2015)
# merge endline data id number and group column from random assignment data based on respondent id/id number
df = pd.merge(df_e[ ['idno'] ], df_a[ ['RespID', 'Give'] ], how='inner', left_on="idno", right_on="RespID")
...
d2015 = {'Cash': bygroupdf.at['Cash','idno'],'Control': bygroupdf.at['Control','idno'],'TUP': bygroupdf.at['TUP','idno'],'All': df.Give.notna().sum()}
...
df_b_m = pd.merge(df_b[ ['idno'] ], df_m[ ['idno', 'Give'] ], how='inner', left_on="idno", right_on="idno")
bygroupdf_b_m = df_b_m.groupby(['Give']).count()

df_b_e = pd.merge(df_b[ ['idno'] ], df_e[ ['idno', 'Give'] ], how='inner', left_on="idno", right_on="idno")
bygroupdf_b_e = df_b_e.groupby(['Give']).count()

dd2013 = d2013
dd2014 = {'Cash': bygroupdf_b_m.at['Cash','idno'],'Control': bygroupdf_b_m.at['Control','idno'],'TUP': bygroupdf_b_m.at['TUP','idno'],'All': df_b_m.Give.notna().sum()}
dd2015 = {'Cash': bygroupdf_b_e.at['Cash','idno'],'Control': bygroupdf_b_e.at['Control','idno'],'TUP': bygroupdf_b_e.at['TUP','idno'],'All': df_b_e.Give.notna().sum()}
...
#+RESULTS: checklist_20150602.csv_attrition
|         |    2013 |    2014 |    2015 |
|---------+---------+---------+---------|
| Cash    | 124.000 | 113.000 | 111.000 |
| Control | 280.000 | 266.000 | 347.000 |
...
#+name: master_assignment.csv_attrition
#+begin_src python :exports none  :results output table raw labels=[] :colnames no
data_baseline  = "../../TUP-data/Baseline/TUP_baseline.dta"
...
#+RESULTS: master_assignment.csv_attrition
|         |    2013 |    2014 |    2015 |
|---------+---------+---------+---------|
| Cash    | 124.000 | 113.000 | 111.000 |
